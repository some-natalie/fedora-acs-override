...
Which Fedora versions get published comes from the `fc*-latest` releases that exist, unioned with whatever built this run, rather than from a hardcoded list. So a Fedora version that goes EOL and drops out of the build jobs keeps its repodata and its last few kernels stay installable, instead of the whole-site deploy quietly removing its directory and leaving everyone still on it with a repo that 404s. `dnf` doesn't skip an unreachable repo by default, so that would break unrelated installs too, not just kernel updates. Retiring a Fedora version for real means deleting its release by hand.

The cost of that is re-downloading every kept RPM on each publish, for every release including the EOL ones, so it grows as versions accumulate - roughly 750 MB per release. If that gets tiresome, the fix is to cache each release's generated `repodata` as an asset on its own release and skip `createrepo_c` when nothing changed, rather than to stop re-indexing.
